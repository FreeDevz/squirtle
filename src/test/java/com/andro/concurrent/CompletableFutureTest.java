package com.andro.concurrent;

import com.andro.functional.throwing.ThrowingSupplier;
...
public class CompletableFutureTest {
    ...
    @Test
    public void test_run_async_get() throws Exception {
        CompletableFuture<Void> runAsync = CompletableFuture.runAsync(() -> System.out.println("running async task"), service);
        runAsync.get();
        assertThat(runAsync.isDone(), is(true));
    }
    ...
    @Test
    public void test_my_use_result() throws Exception {
        //CompletableFuture<String> task = CompletableFuture.supplyAsync(simulatedTask(5, "result after 5 second"), service);
        CompletableFuture<String> task = CompletableFuture.supplyAsync(() -> {
            ...
            return "result after 5 second";
        });
        String result = task.get();
        System.out.println("Got result! : " + result);
        assertThat("result after 5 second", is(result));
    }
    ...
    @Test
    public void test_my_run_after_both() throws Exception {

        CompletableFuture<String> run1 = CompletableFuture.supplyAsync(() -> {
            int i = 0;
            while (i < 2) {
                try {
                    Thread.sleep(1000);
                ...
                System.out.println(Thread.currentThread().getId() + ", first task: sleeping 1 second....");
                i++;
            }
            return "first task";
        }, service);

        CompletableFuture<String> run2 = CompletableFuture.supplyAsync(() -> {
            int i = 0;
            while (i < 3) {
                try {
                    Thread.sleep(1000);
                ...
                System.out.println(Thread.currentThread().getId() + ", second task: sleeping 1 second....");
                i++;
            }
            return "second task";
        }, service);

        CompletableFuture<String> finisher = CompletableFuture.supplyAsync(() -> {
            try {
                System.out.println(Thread.currentThread().getId() + ", both task: sleeping 1 second....");
                return run1.get() + "&" + run2.get();
            } catch (InterruptedException e) {
                e.printStackTrace();
        ...
        String result = finisher.get();
        System.out.println(Thread.currentThread().getId() + ", main task: sleeping 1 second....");
        assertThat(result, is("first task&second task"));
    }
